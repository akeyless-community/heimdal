...
from typing import Callable, TypedDict, Annotated, Sequence
...
import os
import functools, operator, logging, os, json, asyncio
...
from heimdal.tools.utility_tools.kubernetes_operations import fetch_service_account_info, generate_k8s_secret_from_literal_values
...
# class CloudServiceDetector(BaseModel):
#     cloud_service_provider: str = Field(description="Detected cloud service provider")
...
def cloud_service_detector_tool() -> str:
    ...
    try:
        ...
        logger.debug(f"Detected cloud service provider: {cloud_service_provider}")
...
def create_auth_method(auth_type: str, create_method: Callable) -> str:
    ...
    try:
        ...
        if result.startswith("p-"):
            ...
            json_result_str = json.dumps({"message": f"{auth_type} authentication method created.", "akeyless_access_id": result})
        else:
            logger.error(f"Failed to create {auth_type} authentication method.")
            json_result_str = json.dumps({"error": f"Failed to create {auth_type} authentication method."})
    except Exception as e:
        error_message = {"error": str(e)}
        logger.error(f"Error creating {auth_type} authentication method: " + str(e))
...
class KubernetesSecret(BaseModel):
    ...
    namespace: str = Field(description="Namespace in which the secret will be deployed")
    literal_values: dict = Field(description="Literal values to be used in the secret")
...
def kubernetes_secret_deployer_tool(secret_name: str, namespace: str, literal_values: dict) -> str:
    ...
    try:
        ...
        result = generate_k8s_secret_from_literal_values(secret_name, namespace, literal_values)
...
class HelmChartDeployment(BaseModel):
    namespace: str = Field(description="Namespace in which the chart will be deployed")
    auth_method_id: str = Field(description="Akeyless access id for auth method")
...
def helm_chart_deployer_tool(namespace: str, auth_method_id: str) -> str:
    ...
    """
    try:
        logger.debug("Deploying Helm chart with auth method id: " + auth_method_id + " in namespace: " + namespace)
        result = asyncio.run(deploy_akeyless_gateway(namespace, auth_method_id))
        return result
    except Exception as e:
        error_message = {"error": str(e)}
...
class Response(BaseModel):
    ...
    namespace: str = Field(description="Namespace in which the chart was deployed")
    service_account: str = Field(description="Service account used to deploy the chart")
    detected_cloud_service_provider: str = Field(description="Detected cloud service provider")
    akeyless_access_id: str = Field(description="Akeyless access id for auth method")
...
## Please check what the kubernetes namespace and service account are for this bot to deploy the akeyless gateway helm chart. Please figure out the cloud service provider where this bot is installed and create the appropriate authentication method. If the authentication method is an API key, please create a kubernetes secret and store the API key in the secret. Once the secret is created, please deploy the helm chart using the secret. If the authentication method is not an API key then please deploy the helm chart using the authentication method access ID. Once the helm chart is deployed, please return all the details about everything created.

inputs = {
    "messages": [
        HumanMessage(
            content="""Please check what the kubernetes namespace and service account are for this bot to deploy the akeyless gateway helm chart. Please figure out the cloud service provider where this bot is installed and create the appropriate authentication method. Please deploy the helm chart using the authentication method access ID. Once the helm chart is deployed, please return all the details about everything created."""
        )
    ]
}
for output in app.with_config({"run_name": "Heimdal with Tools"}).stream(inputs):
