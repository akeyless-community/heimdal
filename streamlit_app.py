            ...
            for key, value in event.items():
                if key == "agent":
                    ...
                    if latest_message.additional_kwargs["tool_calls"][0]["type"] == "function":
                        ...
                        if arguments == "{}":
                            tool_execution_message = f"Running the {function_name} tool with no arguments"
                        else:
                            tool_execution_message = f"Running the {function_name} tool with the arguments {arguments}"
                        
                        st.write(tool_execution_message)
...
def handle_click():
    ...
    asyncio.run(handle_click_async())
